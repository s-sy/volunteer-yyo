...
Page({
      ...
      success: function (res) {
        ...
        orderConfirmData.forEach((orderConfirm, index) => {
          ...
          if (orderConfirm.pointsDeductSwitch == '1') {
            let pointsDeductPrice = Math.ceil(orderConfirm.salesPrice * orderConfirm.pointsDeductScale / 100 * orderConfirm.quantity)
  ...
  orderSub(){
    ...
    let deliveryPoint = that.data.deliveryPoint
    ...
    if (that.data.orderSubParm.deliveryWay == '2' && deliveryPoint == null){
      wx.showToast({
        title: '请选择提货点',
        icon: 'none',
        duration: 2000
    ...
    app.api.orderSub(Object.assign(
      ...
      deliveryPointId: that.data.orderSubParm.deliveryWay == '2' ? deliveryPoint.id : null
    },
      orderSubParm
    ))
      .then(res => {
  ...
  pointsCheckedChange(e) {
    if (e.detail.value == 'true'){
      ...
      orderConfirmData.forEach(function (orderConfirm) {
        if (orderConfirm.pointsDeductSwitch == '1') {
          orderConfirm.paymentPrice = (orderConfirm.paymentPrice2 - that.data.totalCouponDeductPrice) < 0 ? 0 : orderConfirm.paymentPrice2 - that.data.totalCouponDeductPrice
      ...
      this.setData({
        ...
        paymentPrice: (Number(this.data.salesPrice) - Number(this.data.totalCouponDeductPrice) - Number(this.data.totalPointsDeductPriceTemp)).toFixed(2) < 0 ? 0 : (Number(this.data.salesPrice) - Number(this.data.totalCouponDeductPrice) - Number(this.data.totalPointsDeductPriceTemp)).toFixed(2),
        orderConfirmData: orderConfirmData
      })
    }else{
      let orderConfirmData = this.data.orderConfirmData
